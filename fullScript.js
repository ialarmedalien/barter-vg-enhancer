/*
 * Copyright (C) 2017 Alexander Krivács Schrøder <alexschrod@gmail.com>, 2023 ialarmedalien
 *
...
// @namespace    https://github.com/ialarmedalien/
// @version      0.5.4
// @description  Summarizes and compares all attributes in an offer for easy comparison of offer value
// @homepage     https://github.com/ialarmedalien/barter-vg-enhancer
// @author       Alexander Krivács Schrøder
// @author       ialarmedalien
// @updateURL    TODO
// @downloadURL  https://github.com/ialarmedalien/barter-vg-enhancer/barter-vg-enhanced.user.js
// @supportURL   https://github.com/ialarmedalien/barter-vg-enhancer/issues
...
class Router {
	...
	route() {
		...
		if (chosenRoute !== null) {
			let controller = new chosenRoute.controller();
...
function getHeaderFunction(headers) {
	let keyedHeaders = {};
	jquery.each(headers, function(_, value) {
		let splitValue = value.trim().split(':', 2);
	...
	static get(url, callback) {
		GM_xmlhttpRequest({
			...
			onload: function(gmResponse) {
				let headers = getHeaderFunction(gmResponse.responseHeaders.split('\n'));
...
class Steam {
	static getPricesFor(steamAppIds, callback) {
		let gamePrices = {};
		jquery.each(steamAppIds, function(index, steamAppId) {
			// Check if the game's price has been retrieved lately
			let cachedPriceJson = Greasemonkey.getValue('priceCache.' + steamAppId, null);
			if (cachedPriceJson === null) {
				return;
			}

			let cachedPrice = JSON.parse(cachedPriceJson);
			let cacheAge = new Date() - new Date(cachedPrice.freshTime);
		...
		jquery.each(gamePrices, function(steamAppId) {
			let index = steamAppIds.indexOf(steamAppId);
			if (index === -1) {
				return;
			}
		...
		let urlQuery = jquery.param(urlParameters);
		if (urlQuery) {
			url += '?' + urlQuery;
		}
		...
		let cb = function(success, response) {
			...
			if (success) {
				jquery.each(response.data, function(key, datum) {
					if (!datum.success) {
...
variables.css.style = '.bve-game-details{position:absolute;top:0;right:15px;text-align:right}.bve-game-details__steam-store-price,.bve-game-details__trade-ratio{font-size:100%}.bve-trade-summary{width:100%}.bve-information-highlight{border-bottom:dotted 1px;cursor:help;font-size:16px}\n';

variables.html.gameDetails = '<div class="bve-game-details"><span><span class="bve-game-details__steam-store-price">Loading...</span></span><br><span>H:W Ratio: <span class="bve-game-details__trade-ratio">{0}</span></span></div>';

variables.html.tradeSummary = '<table class="bve-trade-summary"><caption>Trade summary:</caption><tr><th>Games in bundles</th><td>{1}</td></tr><tr><th>Total bundles</th><td>{8}</td></tr><tr><th>Average review score <span title="The more reviews it has, the proportionally larger that game\'s impact on the score" class="bve-information-highlight">(weighted)</span></th><td>{4}% ({5}%)</td></tr><tr><th>Number of reviews <span title="The binary logarithm of the number of reviews. A difference of +1 means &quot;twice as popular&quot;, and -1 means &quot;half as popular&quot;." class="bve-information-highlight">(log<sub>2</sub>)</span></th><td>{10} ({11})</td></tr><tr><th>Trade ratio (H : W)</th><td>{6} ({9})</td></tr><tr><th>Total price on Steam (ignoring any active discounts)</th><td id="{12}_total_value">Loading...</td></tr><tr><th>Average price per game on Steam (ignoring any active discounts)</th><td id="{12}_average_value">Loading...</td></tr></table>';
...
function calculateGameStats(games) {
	const gameStats = games.reduce(gameStatReducer, jquery.extend({}, GAME_STAT_TEMPLATE_OBJECT));
...
function addElementToGameOffers(tradeables, gameOffers) {
	...
	jquery.each(tradablesItemsList, (_, tradablesItems) => {
		const gameUrl = jquery(tradablesItems).find('.tradables_info > strong > a').attr('href');
		const urlRegex = /https:\/\/barter.vg\/i\/(\d+)\//;
		const match = urlRegex.exec(gameUrl);
		if (match !== null) {
			let gameOffer = gameOffers.find(go => {
...
function addGameDetails(gameOffers) {
	gameOffers.forEach(gameOffer => {
		...
		jquery(gameElement).css('position', 'relative');
		jquery(gameElement).find('.tradables_info').css('max-width', '380px');
		jquery(gameElement).append(format(variables.html.gameDetails, ratios.summary));
		let steamStorePriceElement = jquery(gameElement).find('.bve-game-details__steam-store-price');
		let tradeRatioElement = jquery(gameElement).find('.bve-game-details__trade-ratio');
...
class OffersPageController {
	static get routes() {
		return [
			new Route(
				...
				() => jquery('.statusCurrent').text() !== 'Creating...'
			)
		];
	}
	...
	index() {
		Http.get(`${window.location.href}json`, (result, response) => {
			...
			const tradeables = jquery('.tradables');
			...
			const fromTotalValue = jquery(`#${fromIdPrefix}_total_value`);
			const toTotalValue = jquery(`#${toIdPrefix}_total_value`);
			const fromAverageValue = jquery(`#${fromIdPrefix}_average_value`);
			const toAverageValue = jquery(`#${toIdPrefix}_average_value`);
			...
			Steam.getPricesFor(steamIds, (priceResult, gamePrices) => {
					...
					for (let gpi in gamePrices) {
						...
						if (gamePrices.hasOwnProperty(gamePriceIndex)) {
							...
							if (gamePricesInfo.final === 0) {
								jquery(gameOffer.steamStorePriceElement).html('Free').css('color', 'green');
							} else if (gamePricesInfo.discount_percent === 0) {
								jquery(gameOffer.steamStorePriceElement).html(format('{0} {1}', gamePricesInfo.final / 100.0, currency));
							} else {
								jquery(gameOffer.steamStorePriceElement).html(format('{0} {1} ({2}% off)', gamePricesInfo.final / 100.0, currency, gamePricesInfo.discount_percent));
							}
				...
				gameOffers.filter(go => go.steamId === null).forEach(go => {
					if (priceResult) {
						jquery(go.steamStorePriceElement).html('N/A').css('color', 'darkgray');
					} else {
						jquery(go.steamStorePriceElement).html('Fetching prices failed!').css('color', 'red');
					}
				});
...
var controllers = [OffersPageController];

jquery('head').append(jquery('<style type="text/css">' + variables.css.style + '</style>'));
